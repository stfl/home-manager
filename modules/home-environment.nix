...

{
  ...
  config = {
    ...
    home.activation.writeBoundary = hm.dag.entryAnywhere ''
      dryRunNullOnReal nix-store --realise "$newGenPath" --add-root "$newGenGcPath"
    '';
    ...
    home.activationPackage =
      let
        ...
        activationScript = pkgs.writeShellScript "activation-script" ''
          ...
          dryRunNullOnReal nix-store --realise "$newGenPath" --add-root "$currentGenGcPath"
          dryRun rm $VERBOSE_ARG "$newGenGcPath"
        '';
      in
        pkgs.runCommand
